...
static __declspec(noinline) auto CoreGetComponentRegistry()
{
	static struct RefSource
	{
		RefSource()
		{
			auto func = (ComponentRegistry * (*)())lazy(GetProcAddress).safe_cached()(lazy(GetModuleHandleW).safe_cached()(_wxor_(L"CoreRT.dll").c_str()), _xor_("CoreGetComponentRegistry").c_str());
			this->registry = func();
		}
...
auto CoreGetGlobalInstanceRegistry()
{
	static struct RefSource
	{
		RefSource()
		{
			auto func = (InstanceRegistry * (*)())lazy(GetProcAddress).safe_cached()(lazy(GetModuleHandleW).safe_cached()(_wxor_(L"CoreRT.dll").c_str()), _xor_("CoreGetGlobalInstanceRegistry").c_str());
			this->registry = func();
		}
