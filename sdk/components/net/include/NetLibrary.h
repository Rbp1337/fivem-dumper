...
#include <net-base/include/NetAddress.h>

 // hacky include path to not conflict with our own NetBuffer.h
#include <net-base/include/NetBuffer.h>
...
public:
	...
	fwString GetAddress()
	{
		if (m_type == NA_INET4)
		{
			...
			lazy(inet_ntop).safe_cached()(m_in4.sin_family, &m_in4.sin_addr, buffer, _countof(buffer));

			return buffer;
		}

		return _xor_("unknown").c_str();
	}
	...
	int GetPort()
	{
		if (m_type == NA_INET4)
		{
			return lazy(ntohs).safe_cached()(m_in4.sin_port);
		}
		else if (m_type == NA_INET6)
		{
			return lazy(ntohs).safe_cached()(m_in6.sin6_port);
		}
